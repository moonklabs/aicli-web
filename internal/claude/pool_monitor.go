...
func (m *PoolMonitor) checkAlerts() {
	...
	usedMemory := m.systemMetrics.UsedMemory
	m.systemMutex.RUnlock()

	// CPU 사용률 알람
	if m.config.AlertThresholds.HighCPUUsage > 0 &&
		cpuUsage > m.config.AlertThresholds.HighCPUUsage {
	...
	if m.config.AlertThresholds.HighMemoryUsage > 0 &&
		usedMemory > m.config.AlertThresholds.HighMemoryUsage {
		m.triggerAlert(AlertEvent{
