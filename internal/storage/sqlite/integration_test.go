...
func testWorkspaceOperations(t *testing.T, s storage.Storage) {
	...
	// ExistsByName 테스트
	exists, err := ws.ExistsByName(ctx, workspace.OwnerID, "Updated Workspace Name")
	require.NoError(t, err)
...
func testProjectOperations(t *testing.T, s storage.Storage) {
	...
	err = ps.Update(ctx, updated.ID, configUpdates)
	require.NoError(t, err)

	// Config 업데이트 확인
	configUpdated, err := ps.GetByID(ctx, updated.ID)
	require.NoError(t, err)
	assert.Equal(t, "python setup.py build", configUpdated.Config.BuildCommand)
...
func testSessionOperations(t *testing.T, s storage.Storage) {
	...
	// List를 사용하여 프로젝트별 세션 조회
	filter := &models.SessionFilter{
		ProjectID: project.ID,
	}
	response, err := ss.List(ctx, filter, pagination)
	require.NoError(t, err)
	assert.NotNil(t, response)
	assert.Greater(t, response.Meta.Total, 0)
	sessions := response.Data.([]*models.Session)
	...
	session.CommandCount = int64(10)
	err = ss.Update(ctx, session)
	require.NoError(t, err)
...
func testTaskOperations(t *testing.T, s storage.Storage) {
	...
	// GetByStatus 테스트 - List로 대체
	taskFilter := &models.TaskFilter{
		Status: &[]models.TaskStatus{models.TaskStatusRunning}[0],
	}
	runningTasks, runningTotal, err := ts.List(ctx, taskFilter, pagination)
	require.NoError(t, err)
	assert.Greater(t, runningTotal, 0)
	...
	task.Duration = int64(5000) // 5초
	task.Output = "Build successful"
	err = ts.Update(ctx, task)
	require.NoError(t, err)
...
func testConcurrentOperations(t *testing.T, s storage.Storage) {
	...
	expectedTotal := numGoroutines * numOperationsPerGoroutine
	assert.GreaterOrEqual(t, total, expectedTotal, "Expected at least %d workspaces, got %d", expectedTotal, total)
	assert.GreaterOrEqual(t, len(allWorkspaces), expectedTotal, "Expected at least %d workspaces in result, got %d", expectedTotal, len(allWorkspaces))
}
...
func TestSQLiteStorageConnectionManagement(t *testing.T) {
	...
	// 종료 후 HealthCheck는 실패해야 함 - 메서드가 없어 스킵
	// err = storage.HealthCheck(context.Background())
	// assert.Error(t, err)
}
...
func TestSQLiteStorageWithExistingDB(t *testing.T) {
	...
	row := sqliteStorage.getDB().QueryRow("SELECT name FROM temp_test WHERE id = ?", "test-1")
