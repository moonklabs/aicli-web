...
5. **웹 개발 완료 시 검증**:
   ...
   - 타입스크립트 코드는 반드시 `pnpm tsc` 실행하여 타입 오류 확인
   - 빌드와 린트에서 발생하는 모든 오류를 수정해야 함

6. **통합 검증 (루트 폴더에서 실행)**:
   - **전체 검증**: `pnpm verify` - Go 백엔드와 웹 프론트엔드 모두 검증
   - **빠른 검증**: `pnpm verify:quick` - 린트와 타입 체크만 수행
   - **완전 검증**: `pnpm verify:all` - 빌드, 린트, 테스트, 통합 테스트 모두 수행
   - **테스트 서버**: `pnpm start:test` - 테스트 서버 실행
   - **통합 테스트**: `pnpm test:integration` - 전체 시스템 통합 테스트
